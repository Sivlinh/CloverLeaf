...
export default function Profile() {
  ...
  const addToOrderHistory = (product, quantity = 1) => {
    const order = {
      id: Date.now(),
  ...
  const getStats = () => {
    ...
    const totalSpent = orderHistory.reduce(
      (sum, order) => sum + (order.total || order.price),
      0
    );
    const avgRating =
      favorites.length > 0
        ? products
            ...
            .reduce((sum, p) => sum + p.rating, 0) / favorites.length
        : 0;
        ...
        try {
          ...
          if (data.status === 'PAID') {
            ...
            setTimeout(() => {
              const updatedUser = {
                ...user,
                wallet: (user.wallet || 0) + parseFloat(topUpAmount),
              };
  ...
  const stats = user
    ? getStats()
    : { totalOrders: 0, totalSpent: 0, avgRating: 0 };
                  ...
                  {orderHistory.length > 0 && (
                    <div className="bg-gradient-to-r from-green-50 to-blue-50 p-4 rounded-lg mb-6">
                      <div className="flex justify-between items-center">
                        <div>
                          <h4 className="font-semibold text-gray-800">Total Money Spent</h4>
                          <p className="text-sm text-gray-600">{orderHistory.length} orders</p>
                        </div>
                          ...
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex items-center gap-4">
                                <img
                                  src={product && product.images && product.images[0] ? product.images[0] : "/logo_shop.png"}
